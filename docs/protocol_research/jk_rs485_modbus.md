...
Status: active safe-read implementation with hardware-confirmed FC03 cell
voltage probes at `0x1200`. Decode is limited to confirmed cell-voltage
register reads.
...
Hardware-confirmed active cell-voltage probes:

- `read_cell_voltage_0_addr1`: slave `0x01`, FC03, start register `0x1200`,
  quantity `0x0001`, TX `01 03 12 00 00 01 81 72`. Primary sanity probe.
- `read_cell_voltages_0_7_addr1`: slave `0x01`, FC03, start register `0x1200`,
  quantity `0x0008`, TX `01 03 12 00 00 08 41 74`. Primary cell-voltage block.
- `read_cell_voltages_0_15_addr1`: slave `0x01`, FC03, start register
  `0x1200`, quantity `0x0010`, TX `01 03 12 00 00 10 41 7E`. Extended
  cell-voltage block.
...
- Date: 2026-05-13
- BMS menu protocol: `013 (9600) JK BMS RS485 Modbus V1.0`
- Serial: 9600 8N1
- Probe: `read_cell_voltage_0_addr1`
- TX: `01 03 12 00 00 01 81 72`
- RX: `01 03 02 0C D8 BD 1E`
- CRC: OK
- Slave ID: OK
- Function code: OK
- Byte count: OK
- Register `0x1200` raw: 3288
- Probable value: Cell voltage 0 = 3288 mV = 3.288 V
- Status: hardware-confirmed single read-only probe

Additional hardware-confirmed active probes, per 2026-05-13 operator report:

- `read_cell_voltages_0_7_addr1`, FC03, register `0x1200`, quantity 8.
- `read_cell_voltages_0_15_addr1`, FC03, register `0x1200`, quantity 16.
- `read_cell_voltages_0_7_addr1` TX:
  `01 03 12 00 00 08 41 74`.
- `read_cell_voltages_0_7_addr1` RX:
  `01 03 10 0C D8 0C D7 0C D7 0C D7 0C D8 0C D7 0C D8 0C D8 72 06`.
- `read_cell_voltages_0_7_addr1` decoded active voltages:
  3288, 3287, 3287, 3287, 3288, 3287, 3288, 3288 mV.
- `read_cell_voltages_0_15_addr1` TX:
  `01 03 12 00 00 10 41 7E`.
- `read_cell_voltages_0_15_addr1` RX:
  `01 03 20 0C D7 0C D7 0C D7 0C D7 0C D7 0C D7 0C D8 0C D7 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 26 C8`.
- `read_cell_voltages_0_15_addr1` decoded active voltages:
  3287, 3287, 3287, 3287, 3287, 3287, 3288, 3287 mV.
- `read_cell_voltages_0_15_addr1` trailing zeros at registers 8..15 are treated
  as unused registers.
- Repository RX logs:
  `logs/manual_tx_20260513T195227Z.json` and
  `logs/manual_tx_20260513T195405Z.json`.

Cell-voltage block decode policy:

- Values `2500..4500` mV are active cell voltages.
- Trailing zeros after active cells are unused registers.
- Zeros are not included in min/max/delta.
- A zero between non-zero register values is reported as
  `non_trailing_zero_cell_voltage`.

## Active safe-read implementation plan

- Keep `JkRs485ModbusProfile` importable from `protocols.jk_rs485_modbus`.
- Return it from `protocols.get_all_profiles()` as a distinct profile from
  `jk_pylon_lv_emulation`.
- `enabled_by_default = True`.
- Only confirmed FC03/FC04 read request building is available.
- Active probes are limited to confirmed FC03 cell-voltage reads at `0x1200`
  with quantities 1, 8, and 16.
- Wider or unrelated windows remain `unverified_read` metadata and must not be
  sent by scan.
- No write/control helpers, probes, or CLI scan support.
- Decode is intentionally limited to hardware-confirmed `0x1200` cell-voltage
  responses. No wider field-level parser is active.

## Limits

- This confirms only slave 1, FC03, register `0x1200`, quantities 1, 8, and 16.
- It does not confirm the full JK register map.
- Wide read windows must remain research/inactive until hardware-tested.
- No write/control/config/factory commands are allowed.
